...
export function Signin() {
  ...
  async function signin() {
    ...
    try {
        const response = await axios.post(`${BACKEND_URL}/api/v1/braino/signin`, {
            username,
            password
        });
        ...
        alert("Error signing in. Username might be taken.");
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 font-['Poppins'] relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-md border border-white/50 rounded-3xl shadow-2xl p-12 relative z-10">
        <div className="mb-8 text-center">
            ...
            <p className="text-gray-500 mt-2">Signin in your second brain and lookup the importants.</p>
        </div>
        ...
        <div className="flex justify-center mt-8">
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}>
             <Button title={"Sign In"} variant="submission" onClick={signin} />
          </motion.div>
        </div>
         <p className="text-center mt-6 text-gray-600">
            Don't have an account? <span onClick={() => navigate('/signup')} className="text-blue-500 cursor-pointer hover:underline">Sign up</span>
        </p>
      </div>
    </div>
  );
